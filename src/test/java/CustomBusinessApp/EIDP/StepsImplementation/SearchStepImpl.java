...
public class SearchStepImpl extends PageInitializer{
	...
	public void selectStatus(String statusOption) {
		...
		CommonUtils.click(WebDriverUtils.getWebDriver().findElement(By.cssSelector(".select2-search.select2-search--dropdown .select2-search__field")));
		CommonUtils.sendKeys(WebDriverUtils.getWebDriver().findElement(By.cssSelector(".select2-search.select2-search--dropdown .select2-search__field")), statusOption);
		CommonUtils.click(WebDriverUtils.getWebDriver().findElement(By.xpath("//li[@role='treeitem'][text()=\""+statusOption+"\"]")));
	...
	public void selectActiveCompletedIDP() throws Exception {
		...
		List<WebElement> activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary initiateAnother'][not(@disabled)]"));
		Boolean isSelected = false;
		while(!isSelected) {
			activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary initiateAnother'][not(@disabled)]"));
	...
	public void selectReleaseHoldIDP() {
		CommonUtil.waitBrowser(4000);
		CommonUtils.waitForVisibility(WebDriverUtils.getWebDriver().findElement(By.id("advanced_search_results")));
		List<WebElement> activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary holdIDP'][not(@disabled)]"));
		Boolean isSelected = false;
		while(!isSelected) {
			activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary holdIDP'][not(@disabled)]"));
			if(activeButtons.size() > 0) {
				CommonUtils.click(activeButtons.get(0));
				isSelected = true;
			} else {
	...
	public void selectCancelIDP() {
		CommonUtil.waitBrowser(4000);
		CommonUtils.waitForVisibility(WebDriverUtils.getWebDriver().findElement(By.id("advanced_search_results")));
		List<WebElement> activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary'][not(@disabled)]"));
		Boolean isSelected = false;
		while(!isSelected) {
			activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary'][not(@disabled)]"));
			if(activeButtons.size() > 0) {
				CommonUtils.click(activeButtons.get(0));
				isSelected = true;
			} else {
	...
	public void selectUndoCancelIDP() {
		CommonUtil.waitBrowser(4000);
		CommonUtils.waitForVisibility(WebDriverUtils.getWebDriver().findElement(By.id("advanced_search_results")));
		List<WebElement> activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary undoCancelledIDP'][not(@disabled)]"));
		Boolean isSelected = false;
		while(!isSelected) {
			activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary undoCancelledIDP'][not(@disabled)]"));
			if(activeButtons.size() > 0) {
				CommonUtils.click(activeButtons.get(0));
				isSelected = true;
			} else {
	...
	public void selectExistSurveyIDP() {
		CommonUtil.waitBrowser(4000);
		CommonUtils.waitForVisibility(WebDriverUtils.getWebDriver().findElement(By.id("advanced_search_results")));
		List<WebElement> activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary exitSurvey'][not(@disabled)]"));
		Boolean isSelected = false;
		while(!isSelected) {
			activeButtons = WebDriverUtils.getWebDriver().findElements(By.xpath("//button[@class='btn btn-primary exitSurvey'][not(@disabled)]"));
			if(activeButtons.size() > 0) {
				CommonUtils.click(activeButtons.get(0));
				isSelected = true;
			} else {
	...
	public String getTrainneeNameFromHoldConfirmationWindow() {
		return WebDriverUtils.getWebDriver().findElement(By.cssSelector(".modal.modal-lg.fade.in .traineeDetails > a")).getText();
	}
